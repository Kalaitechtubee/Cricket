...
const Scoreboard = memo(({ match }) => {
  ...
  const { currentRunRate, battingStats, bowlingStats, matchStatus } = match.scoreboard || {};
  const isLive = match.status === "live";
  ...
  return (
    <div className="mt-6">
      ...
      <div className="p-4 bg-gray-900 rounded-lg text-center">
        ...
        <div className="text-white font-bold text-xl">
          {match.teams[0].shortName} {match.teams[0].score}/{match.teams[0].wickets} ({match.teams[0].overs})<br />
          {match.teams[1].shortName} {match.teams[1].score}/{match.teams[1].wickets} ({match.teams[1].overs}) CRR: {currentRunRate}
        </div>
      </div>
